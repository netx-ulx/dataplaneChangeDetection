...
register<int<32>>(SKETCH_WIDTH) forecast_sketch0; 
register<int<32>>(SKETCH_WIDTH) forecast_sketch1; 
register<int<32>>(SKETCH_WIDTH) error_sketch0; // value for the count sketch
register<int<32>>(SKETCH_WIDTH) error_sketch1; // value for the count sketch

register<bit<1>>(SKETCH_WIDTH) control_flag; // 1 bit flag sketch
...
control MyIngress(inout headers hdr,
    ...
    action cal_hash() {
	hash(meta.hash, HashAlgorithm.crc32, 32w0, {meta.flowkey1, meta.flowkey2}, 11w19);
    }
	...
	if (hdr.ipv4.isValid()) {
		...
		sketch_key0.read(meta.tempkey1, meta.hash);
		if (meta.tempkey1 != meta.flowkey1) {
		    meta.flag = 1;
		}
		sketch_key1.read(meta.tempkey2, meta.hash);
		...
		sketch_key0.write(meta.hash, meta.flowkey1);
		sketch_key1.write(meta.hash, meta.flowkey2);
		sketch_count.read(meta.tempcount, meta.hash);
