...
def cycle(arguments):
    ...
    for threshold in thresholds: 
        ...
        #print([false_positives/len(complex_result),mean(accuracies),false_positives,attacks_found,[alpha,threshold,key_format]])
        sys.stdout.flush()
        #If the attack was not found for a given threshold, skip same configuration for bigger thresholds
        if attacks_found == 0:
            break
        results.append([false_positives/len(complex_result),mean(accuracies),false_positives,attacks_found,[alpha,threshold,key_format]])
    return results
...
def main():
    ...
    for result in sorted_results:
        if result[3] > 0:
            if best == None:
                print("Best combination:",result[0],result[1],result[2],result[3],result[4])
                best = result[0]
                #print("Sorted Results:",sorted_results)
            elif result[0] == best:
                print("Other best combinations:",result[0],result[1],result[2],result[3],result[4])
            else:
                break
    ...
    for result in sorted_results:
        if result[3] > 0:
            if best == None:
                print("Best Accuracy combination:",result[0],result[1],result[2],result[3],result[4])
                best = result[0]
                #print("Sorted Results:",sorted_results)
            elif result[0] == best:
                print("Other Accuracy best combinations:",result[0],result[1],result[2],result[3],result[4])
            else:
                break
