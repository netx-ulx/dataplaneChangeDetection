...
class Window:
    ...
    def get_microbursts(self,factor,options=None):
        microbursts = []
        for k in range(1,len(self.averages)):
            for t in range(0,len(self.averages[0])):
                #Insert Criteria for Microburst:
                if self.values[k][t] > factor*self.averages[k][t]:
                    list = self.keys[k].split(' ')[-1].replace('\'','').replace('}','')
                    #print(str(list) + ' ' + str(self.time[t]), end=' ')
        ...
        if action == "add":
            ...
            if option == "microbursts":
                try:
                    ...
                    factor = args[0]
                    cur_trace.cur_window.get_microbursts(float(factor))
                finally:
                    continue
            elif option == "f2":
                try:
