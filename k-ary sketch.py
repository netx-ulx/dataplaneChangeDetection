...
def EWMA(forecast_sketch,observed_sketch,alpha):
    ...
    new_forecast_sketch = KAry_Sketch(depth,width)
    if forecast_sketch != None:
        for i in range(0,depth):
            for j in range(0,width):
                new_forecast_sketch.sketch[i][j] = alpha*observed_sketch.sketch[i][j] + ((1-alpha)*forecast_sketch.sketch[i][j])
        return new_forecast_sketch
    else:
        return copy.deepcopy(observed_sketch)
...
def change(forecast_sketch,observed_sketch):
...
def main():
    ...
    path = "traces/trace1.pcap"
    #weights = [0.7,0.3] #keeps the weights given to the past past sketches in some forecasting models (must be the size of w)
    alpha = 0.7 #alpha to be used by the EWMA
    w = 1 #number of past sketches saved for forecast ( =1 for EWMA)
    key_format = ["src","dst","dport","sport","proto"]
    keys = set()

    #-------------------------------------------- PROCESS INPUT --------------------------------------------#
    short_options = "hd:w:e:a:k:p:"                                                                                                                                         
    long_options = ["help", "depth=", "width=", "epoch=", "alpha=", "key=", "path="]
    ...
    for packet in packets:
        ...
        if cur_epoch < packet["time"] - kary_epoch:
            ...
            if t > 1:
                ...
                #CHANGE DETECTION
                error_sketch, threshold = change(forecast_sketch,sketch_list[-1])
                #error_sketch.SHOW()
