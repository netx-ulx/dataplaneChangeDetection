...
def cycle(arguments):
    ...
    for threshold in thresholds:
        ...
        for epoch in complex_result:
            for change in epoch["res"]:
                ...
                for target in targets:
                    if float(epoch["epoch"][1][1]) <= float(target[1]) and float(epoch["epoch"][1][1]) + kary_epoch >= float(target[1]): #If known attack is present in the changes for its epoch   
        ...
        if detections == 0:
            break
        detections = detections + outoftime
        precision = detections / (detections + false_positives)
        recall = 1

        if arguments[10] == "nshw":
            print([round(false_positives/len(complex_result),3),round(mean(accuracies),3),round(precision,3),round(recall,3),false_positives,detections,attacks_found,len(cycle_targets),[alpha,beta,threshold,key_format]])
            results.append([false_positives/len(complex_result),mean(accuracies),precision,recall,false_positives,detections,[alpha,beta,threshold,key_format]])
        else:
            print(["{:.3f}".format(false_positives/len(complex_result)),"{:.3f}".format(mean(accuracies)),"{:.3f}".format(precision),"{:.3f}".format(recall),false_positives,detections,attacks_found,[kary_epoch,alpha,beta,threshold,key_format],speed])
            #print([false_positives/len(complex_result),mean(accuracies),precision,recall,false_positives,detections,attacks_found,len(cycle_targets),[kary_epoch,alpha,threshold,key_format],speed])
            results.append([false_positives/len(complex_result),mean(accuracies),precision,recall,false_positives,detections,[kary_epoch,alpha,threshold,key_format],speed])
    return results
...
def main():
    ...
    for result in sorted_results:
        if result[3] > 0:
            if best == None:
                print("Best combination:",result[0],result[1],result[2],result[3],result[4],result[5],result[6],result[7])
                best = result[0]
                #print("Sorted Results:",sorted_results)
            elif result[0] == best:
                print("Other best combinations:",result[0],result[1],result[2],result[3],result[4],result[5],result[6],result[7])
            else:
                break
    ...
    for result in sorted_results:
        if result[3] > 0:
            if best == None:
                print("Best Accuracy combination:",result[0],result[1],result[2],result[3],result[4],result[5],result[6],result[7])
                best = result[0]
                #print("Sorted Results:",sorted_results)
            elif result[0] == best:
                print("Other Accuracy best combinations:",result[0],result[1],result[2],result[3],result[4],result[5],result[6],result[7])
            else:
                break
